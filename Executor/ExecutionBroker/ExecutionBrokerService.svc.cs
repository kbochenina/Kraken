...
namespace MITP
{
    ...
    {
        private const string WF_BAG_SIZE = "WfBagSize";

        //TODO: it is possible that Dictionary can be replaced by a Set in the future. Feel comfortable to do it after checking the existing use cases. 
        private static readonly Dictionary<string/*wfId*/, HashSet<ulong/*taskId*/>> _submittedWfs = new Dictionary<string, HashSet<ulong>>();
        ...
        public void DefineDependencies(IEnumerable<TaskDependency> dependencies)
        {
            ...
            //TODO: crutch for Claudia
            RegisterCompletingOfWfFromDependencies(dependencies);
        }
                            ...
                            (DateTime.Now < _lastUpdateTime))) // system time was decreased
                    {
                        ...
                        try
                        {
                            ...
                            lock (_queueLock)
                            {
                                ...
                                // Only tasks belong to completely submitted wf's are allowed to go in the task queue
                                var allowedToSchedule = _definesQueue.Where(t => _submittedWfs.ContainsKey(t.WfId)).ToList();
                                foreach (var task in allowedToSchedule)
                                {
                                    _definesQueue.Remove(task);
                                    _tasksQueue.RemoveAll(t => t.TaskId == task.TaskId);
                                    _tasksQueue.Add(task);
                                    Log.Info(String.Format("Task {0} ({1}) oper-queued", task.TaskId, task.Package));
                                }
            ...
            IEnumerable<TaskDependency> dependencies)
        {
            lock (_updateLock)
            {
                ...
                {
                    var not_finished_tasks = tasks.Where(t => !t.IsFinished()).ToList();
                    var not_finished_tasks_of_full_wfs =
                        not_finished_tasks.Where(t => _submittedWfs.ContainsKey(t.WfId)).ToList();

                    // check count of submitted workflows if the appropriate setting is provided
                    var wf_bag_size = 0;
                    if (ConfigurationManager.AppSettings[WF_BAG_SIZE] != null &&
                        Int32.TryParse(ConfigurationManager.AppSettings[WF_BAG_SIZE], out wf_bag_size))
                    {
                        // allow to process only if the count satisfies the presented condition
                        not_finished_tasks_of_full_wfs = _submittedWfs.Count < wf_bag_size ? new List<Task>() : not_finished_tasks_of_full_wfs;
                    }

                    if (not_finished_tasks_of_full_wfs.Count > 0)
                    {
                        Log.Info("Building new schedule");
                        var estimations = new Dictionary<ulong, Dictionary<NodeConfig, Estimation>>();
                        taskSchedules = TaskSchedule.Build(not_finished_tasks_of_full_wfs, resources, dependencies, out estimations);
        ...
        private void RegisterCompletingOfWfFromDependencies(IEnumerable<TaskDependency> dependencies)
        {
            ...
            var wfIds = dependencies.SelectMany(d => new ulong[] { d.ChildTaskId, d.ParentTaskId }).
                Select(id => _definesQueue.Find(t => t.TaskId == id).WfId).
                Distinct().ToList();

            if (wfIds.Count == 0 && dependencies.Count() == 0)
            {
                //                throw new InvalidDataException("There is no dependencies. Perhaps, because of a workflow consisted of only one task. Currently this situation isn't allowed.");
                //check defined tasks for the single
                var connectdTasksIds = dependencies.SelectMany(d => new ulong[] { d.ChildTaskId, d.ParentTaskId });
                var singleTasks = _definesQueue.FindAll(t => !connectdTasksIds.Contains(t.TaskId)).ToList();
                if (singleTasks.Count == 0 || singleTasks.Count > 1)
                {
                    var str = string.Format(
                        "It seems there is a workflow containing of only one tasks, but found count of such defined tasks: {0}",
                        singleTasks.Count);
            ...
            if (wfIds.Count > 1)
            {
                throw new InvalidDataException(string.Format("Dependecies are contains data for several wfs: {0}. It cannot be sure there are only complete data of all wfs", wfIds));
            }

            var wfId = wfIds.Single();
            if (_submittedWfs.ContainsKey(wfId))
            {
                throw new InvalidDataException("Tryna to add wfId whcih is already contained in the list of completely defined wfs.");
            }

            var wfTasksIds = new HashSet<ulong>(_definesQueue.FindAll(t => t.WfId == wfId).Select(t => t.TaskId));
            _submittedWfs.Add(wfId, wfTasksIds);
        }
    }
}
